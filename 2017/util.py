...
def getArgs():
    ...
    if len(sys.argv) > 2:
        expected_ans = int(sys.argv[2]) ## get test case quickly from command line arguments

    if t:
        inp = sys.stdin.read().strip()
    else:
        inp = open(f'{str(day).zfill(2)}.i').read().strip()

    expected_ans = 1337 # change this to the expected answer for the test case
    # I would advise doing it with the CLI to save time tho
    if len(sys.argv) > 2:
        expected_ans = int(sys.argv[2]) ## get test case quickly from command line arguments
