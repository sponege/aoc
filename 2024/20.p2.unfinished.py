...
for x in range(w):
    for y in range(h):#[[3,0],[2,1],[1,2],[0,3],[-1,2],[-2,1],[-3,0]]
        ...
        size=20
        for cursize in range(1,size+1):
        # cursize=size
        # for i in range(1):
            for dx in range(-cursize,cursize+1):
                for dy in range(cursize+1):
                    if abs(dx) + abs(dy) != cursize: continue
                    nx,ny=x+dx,y+dy
                    a = get(x,y)
                    b = get(nx,ny)
                    diff = a - b
                    ...
                    if diff == 64 and test:
                        ...
                        # ng[ny][nx] = 'e'
                        # print('\n'.join(''.join(l) for l in ng))
                        if test: print((x,y), (nx,ny), a, b, a-b, abs(a-b) - cursize, shortest_path)
            # print((x,y), (nx,ny))
