...
def step(expect=(len(lines[0])-2,len(lines)-1)):
    global blizzards, paths, curRound
    print('expecting', expect)
    real_print(f"Number of paths: {len(paths)}")
    nogo_set = set() ## for fast lookup
    for _ in rl(blizzards):
        x, y, d = blizzards[_]
        dx, dy = getDirMapping(d)
        nx, ny = x+dx, y+dy
        if getPoint(nx, ny) == '#':
            nx = 1 if dx == 1 else len(lines[0])-2 if dx == -1 else x
            ny = 1 if dy == 1 else len(lines)-2 if dy == -1 else y
        blizzards[_] = (nx, ny, d)
        nogo_set.add((nx,ny))
    nps=[]
    for _ in rl(paths):
        ...
        for d in [(0,0),(1,0), (0,1), (-1,0), (0,-1)]:
            nx, ny = x+d[0], y+d[1]
            nogo = (nx, ny) in nogo_set
            #print('not inside blizzard')
            if nogo or nx < 0 or nx > len(lines[0])-1 or ny < 0 or ny > len(lines)-1 or lines[ny][nx] == '#': continue
