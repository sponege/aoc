...
def surrounded_by_lava(_p,op):
    ...
    for d in all_dirs:
        p = (x+d[0],y+d[1],z+d[2])
        if p in lava:
            if p not in visited_lava: covered_lava.append(p)
...
while len(ps) > 0:
    ...
    if 0:#air:
        print(p not in lava, p, covered_lava)
